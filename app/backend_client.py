...
| 附件字节 | ✅ 没有归属可查 |
| 人工修正（corrections）/**归档自己那条** | ✅ 只对自己的行有效 |
| **删自己那条通知**（`DELETE /api/notifications/{id}`） | ✅ 只对自己的行有效（网页任务板的「删除」用它） |
| 发邀请码/验证码、列用户、改机器字段、投递名单、digest-log | ❌ 403 |

所以这里**故意没有** `list_users` / `request_verify_code` / `find_subscribers` 这些方法：
不是"还没实现"，而是客户端本来就不该有这些能力。想加方法之前先回去看那张表 ——
加了之后 e2e 会用 403 把它挡回来。

（客户端**自己不用** `DELETE`：它重抽时判定"不再是通知"的那条走的是 `add_correction`
归档，而不是删除 —— 归档可逆、不丢数据，删除是网页上用户手点的动作。）
