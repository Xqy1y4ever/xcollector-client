"""group_msg_table.40900 的 Protobuf 解析入口。"""
...
from . import group_40900_pb2 as pb
...
def parse_40900(blob: bytes | None) -> Parsed40900:
    if not blob:
        return Parsed40900(ParseStatus.NULL)
    try:
        body = pb.Body()
        body.ParseFromString(blob)
        return Parsed40900(ParseStatus.TYPED, tuple(body.content))
