...
`rule` 是**故意收得比较紧**的确定性抽取器，两道门槛：

1. 必须命中"通知类词"（通知/请/截止/报名/提交/体检/开会/…）才可能建条，**只有时间词不算**
   ——「@张三 明天」「我下周三可能去不了」这类带日期的闲聊不会有任务；
2. **群务一票否决** —— 改群名片/群昵称、进群退群、查收名单、群文件这些是"群怎么用"，
   不是"有人要去做的事"，命中就绝不建条（「请大家按照要求修改群名片」命中"请/要求"，
   光靠通知词拦不住，所以在真实语料里这类占了大头）。

它不花钱、结果可复现，适合先跑通链路；但**判得不够准**（语气、反讽、"这条是通知还是讨论"
这类事它做不了），也可能漏掉用词不典型的真通知。

要判得准就用 `both`：**是不是通知交给模型**，而**相对时间的日期换算交给代码**：

- 判据是「**该进任务板的官方通知**」：发给全群 + 有具体要做的事或要记住的安排 +
  能指出时间地点（或正式通知口径）。个人叮嘱、口头催促、软邀请、纯提醒、链接转发、
  群务都不算 —— 提示词里逐条列了这些情形，并要求模型对每条"不是"写出 `reason`；
...
#### 为什么建议用 `start-client.ps1` 启动（Windows）

pydantic-settings 的优先级是「**环境变量 > `.env` > 默认值**」。如果启动客户端的那个终端
里设过 `CLIENT_*` 之类的变量（粘过一段设置命令、或从别的脚本继承），那么 `.env` 里怎么改
都不生效 —— 页面会提示「这个键在启动的环境变量里也有（环境变量优先，写文件不会生效）」。

`start-client.ps1` 每次启动前把**跟配置同名的**环境变量从这个会话里清掉（键名是从
`Settings.model_fields` 读的，不写死清单），再把生效的几个值打出来，然后启动：

```powershell
.\start-client.ps1                # = --ui
.\start-client.ps1 --once         # 跑一轮（计划任务也可以指向它）
.\start-client.ps1 --status
```

它做的事都摆在屏幕上（清了哪几个键、生效值是什么），不是魔法。

#### 清掉历史误报：`python -m app.tools.rejudge`

判据改过几轮（"有具体的事要做" → "该进任务板的官方通知"）之后，**已经建出来的**那些
误报不会自己消失。而"标为未读 + 重新处理"只能重抽**镜像里认识**的消息 —— 真实情况是
某个用户的板子上 319 条通知里镜像只认识 61 条，另外 258 条它碰不到。

所以有一个专门的工具，**不碰消息、直接按原文重判每一条已建的通知**：

```bash
python -m app.tools.rejudge            # 干跑：只统计会归档哪些（默认）
python -m app.tools.rejudge --apply    # 真做：把判为非通知的 status 改成 archived
```

- 先过确定性的门（群务 / 闲聊）→ **直接判死、不花模型调用**；剩下的才丢给模型，
  用的是当前提示词，和跑链路时同一套；
- 判死的那些**归档**，不删数据（原文与修正历史都在，前端切「已归档」还看得到）；
- 默认干跑，把"会归档多少条、都是些什么"打出来给你看，确认了再加 `--apply`。
...
- **判据变了之后，它会顺手把不再是通知的旧条目归档。** 这是清理历史误报的通道：
  重抽的结论如果是"这不是通知"（比如原来是靠"请/要求"混进来的群务），客户端会把
  那条通知 `status` 改成 `archived`（用户令牌删不了通知，但归档做得到），同时在日志里
  报"归档了几条"。否则判据收紧之后板子上的误报**一条都不会少**，看起来就像"改了没用"。
  归档不丢数据：原文和修正历史都还在，前端切到「已归档」能看到。
...
- **导出库"被占用"不等于坏了。** 导出收尾那一步要把导出库的日志模式切回 DELETE，
  而它需要**独占**：只要有别人正开着这个库（最典型的是**页面在轮询状态**，或还开着
  另一个客户端实例），SQLite 会立刻返回忙。现在的做法是重试几次、还不行就只记一条
  WARNING 并把这一轮的导出标成 `journal_mode_locked`，**不把整轮判失败**（数据已经写完了）；
  页面在一轮跑着的时候也**不再去读导出库**（那几个计数显示成"—"并说明原因）。
  真遇到时：关掉多余的客户端实例即可。
