...
`rule` 是**故意收得比较紧**的确定性抽取器：一条消息必须命中"通知类词"
（通知/请/截止/报名/提交/体检/开会/…）才可能建条，**只有时间词不算** ——
「@张三 明天」「我下周三可能去不了」这类带日期的闲聊不会有任务。
它不花钱、结果可复现，适合先跑通链路；但**判得不够准**（读到语气、反讽、
"这条是通知还是讨论"这类事它做不了），也可能漏掉用词不典型的真通知。

要判得准就用 `both`：**是不是通知交给模型**，而**相对时间的日期换算交给代码**：

- 提示词里直接放一张按消息发送时间算好的日历（本周/下周/再下周、今天/明天/后天、月底），
  模型查表即可 —— 「下周三」「这周天」「月底前」不再靠它自己做日期运算；
- 模型仍然没给出 `due_at` 时，用规则引擎在同一段原文里算出的时间**补上**，
  并在 `extractor` 里记成 `llm+rule`、在候选列表里写明"这个时间是规则给的"
  （详情抽屉里看得到），把握度不超过 0.8 —— 不会假装是模型算的；
- 模型判"不是通知"时会在日志里写下 `reason`（为什么跳过），跳过的原文照常入库，
  所以"少了哪条、为什么少"查得到。
