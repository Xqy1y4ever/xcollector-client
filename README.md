...
以及你的订阅和游标。**订阅是过滤条件**，所以这里会明确告诉你订了几条 ——
一条都没有的话，客户端什么都不会入库。
...
python -m app.main --since-hours 72  # 忽略游标，从 72 小时前重扫
```
...
> 还没有任何订阅时，客户端**一轮什么都不做，而且不会推进游标**。
> 这是刻意的：先启动客户端、再去订阅是很自然的顺序；如果那次空跑把游标推到底，
> 72 小时的存量就被白白烧掉了，而用户订完之后会发现什么都没补上、毫无线索。
...
并覆盖：订阅过滤、游标推进、**游标丢失后靠"已有通知集合"跳过重复抽取**、
