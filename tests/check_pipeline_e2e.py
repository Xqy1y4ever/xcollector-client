...
2. **一条订阅都不需要**：客户端读的是自己账号的聊天记录库，收窄只有本地白名单。
   （订阅是 bot 的东西，决定它抽什么、扇给谁。）
3. **原文落在自己那一层**：客户端写的原文在 `user_raw_message` 里，服务令牌
   按 id 也读不到它（证明它没进共享层）。
...
# —— 曾经就是这样被本机白名单挡掉的（报错只是 outcomes.get("extracted") = None）。
...
def notifications(token: str) -> list[dict]:
    return sync(token, "/api/notifications", params={"status": "all", "limit": 200}).json()[
        "notifications"
    ]
...
async def run_all() -> int:  # noqa: C901
    ...
    try:
        ...
        # 订阅是 bot 的过滤条件：客户端读的是**自己账号**的聊天记录库，
        # 它只需要判断"这条我处理过没有"（镜像）+ 本地白名单。
        ...
        check_true("而且记下了后端的 raw_id（重试要用它）", bool(row.raw_id), str(row.raw_id))
        ...
        check(
            "通知详情里读得回自己那份原文",
            ((sync(token, f"/api/notifications/{notif['id']}").json() or {}).get("raw") or {}).get("id"),
            row.raw_id,
        )
        ...
        print("\n--- 4. 再跑一轮：一条都不重抽 ---")
        report = await run_cycle(backend, settings, db=SourceDatabase(db_path), mirror=mirror)
        check("一条都没再扫（镜像说这条读过了）", report.scanned, 0)
        ...
        # 附件字节按 **md5** 找回（导出库里通常只有 md5 和一段会过期的 CDN 地址）
        ...
        # ----------------------------------------------------------------
        print("\n--- 9. 缺口检测：群静默过久 → 按用户产生告警 ---")
        # 时间线取自**客户端自己的镜像**（不是后端的共享 group_state）。
        ...
        # ----------------------------------------------------------------
        print("\n--- 12. 判据是「没读过」，不是时间：很老的消息照样要读 ---")
        db_old = SCRATCH / f"e2e-old-{RUN}.db"
        mirror_old_path = SCRATCH / f"mirror-old-{RUN}.db"
        make_source_db(db_old, [
            # 20 天前的一条：任何"回看窗口"都不可能覆盖它
            {"msg_id": "900", "ts": BASE_TS - 20 * 24 * 3600, "text": "下周三前交实验报告",
             "content": text_of("下周三前交实验报告")},
            {"msg_id": "901", "ts": BASE_TS, "text": "本周五19:00开会",
             "content": text_of("本周五19:00开会")},
        ])
        ...
        try:
            ...
            check_true("报告里说清了还有多少没读过", report.unread_before >= 2, str(report.unread_before))
            ...
            check("一条都没再处理", report.processed, 0)
        finally:
            await backend_old.close()

        # ----------------------------------------------------------------
        print("\n--- 13. 后端不可达：报错要留下来，不能假装成功 ---")
        bad = make_settings(token, db_path, backend_base_url="http://127.0.0.1:9")
        ...
        try:
            report = await run_cycle(
                ...
                mirror=Mirror(SCRATCH / f"mirror-bad-{RUN}.db"),
            )
            check_true("留下了错误（不是静默跳过）", bool(report.errors), str(report.errors[:1]))
            check_true("每一条都记成 error，而不是假装成功",
